...
- **Platform note (discovered at execution time, 2026-07-28):** `npx create-next-app@latest` installed **Next.js 16.2.12** (the spec named "Next.js 15" as the contemporaneous version at design time — no reason to pin down given `latest` was requested). Two Next.js 16 breaking changes affect this plan's task bodies as written and must be applied when executing them: (1) `middleware.ts` is renamed to **`proxy.ts`**, exporting a function named `proxy` (not `middleware`) — Task 1-1 below is already corrected for this; (2) Route Handler dynamic segments receive `params` as a **Promise** — every `app/api/**/[id]/route.ts` task in Phases 2-6 must destructure with `const { id } = await params` (`{ params }: { params: Promise<{ id: string }> }`), not synchronous access. Apply this correction when executing those not-yet-written tasks; it is not yet reflected in their inline code snippets below since those snippets were written before this discovery.
- **Platform note (discovered at execution time, 2026-07-28): Prisma 7.9.1**
  installed (spec/plan assumed Prisma 5/6-era conventions) — several breaking
  changes from the `prisma-upgrade-v7` reference skill (bundled with this Prisma
  install under `web/.claude/skills/`) apply throughout this plan's remaining tasks:
  - **Generator:** `provider = "prisma-client"` (not `"prisma-client-js"`),
    required `output = "../app/generated/prisma"`, plus `moduleFormat = "cjs"`
    (chosen over ESM-first to avoid forcing `"type": "module"` onto the whole
    Next.js app, which create-next-app did not set up for ESM by default).
  - **Driver adapter is REQUIRED for SQL providers in v7** (not optional as in
    v6) — every Prisma Client instantiation needs `@prisma/adapter-pg` +
    `pg` installed, and construction changes from `new PrismaClient()` to:
    ```typescript
    ...
    ```
    Task 1-1's `lib/prisma.ts` below is already corrected for this.
  - **Import path:** every later task that does `import { PrismaClient } from
    "@prisma/client"` or `import type { Prisma, User, ... } from
    "@prisma/client"` must instead import from `@/app/generated/prisma/client`
    (the `client` sub-entrypoint, not the package root) — the project's `@/*`
    alias resolves to `web/`. `web/.gitignore` already excludes
    `/app/generated/prisma` (build output, regenerated by `prisma generate` /
    `prisma migrate dev`), consistent with not committing generated code.
  - `dotenv` must be an explicit dependency (v7 no longer auto-loads `.env`).
...
- Create: `web/proxy.ts` (Next.js 16 renamed `middleware.ts` → `proxy.ts`, see the
  Global Constraints platform note)
...
- [ ] **Step 4: `web/proxy.ts`** (Next.js 16 name for what was `middleware.ts` in
  Next.js 15 — same mechanism, renamed file + export)
  ```typescript
